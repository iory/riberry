...
class WireCrc {
public:
    WireCrc() : seed(0) { table = generate_crc_table(); }
...
class WireUnpacker {
public:
    enum Error { NONE = 0, INVALID_CRC = 1, INVALID_LENGTH = 2 };

    explicit WireUnpacker(size_t buffer_size)
    ...
    bool hasError() const { return lastError != NONE; }
    ...
    size_t available() const { return isPacketOpen ? 0 : payloadLength - index; }
