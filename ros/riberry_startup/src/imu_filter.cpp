...
public:
    void init(const std::string& filter_type, std::map<std::string, float> filter_params) {
        ...
        if (filter_type == "band_pass") {
            omega = 2.0 * M_PI * filter_params["cutoff_frequency"] /
                    filter_params["sampling_frequency"];
            alpha = std::sin(omega) *
                    std::sinh(std::log(2.0) / 2.0 * filter_params["cutoff_bandwidth"] * omega /
                              std::sin(omega));
            a0_ = 1.0 + alpha;
...
public:
    ...
    void imuCallback(const sensor_msgs::Imu::ConstPtr& msg) {
        ...
        filtered_msg.linear_acceleration.x =
                bqf_.apply_filter(msg->linear_acceleration.x, "accel_x");
        filtered_msg.linear_acceleration.y =
                bqf_.apply_filter(msg->linear_acceleration.y, "accel_y");
        filtered_msg.linear_acceleration.z =
                bqf_.apply_filter(msg->linear_acceleration.z, "accel_z");
