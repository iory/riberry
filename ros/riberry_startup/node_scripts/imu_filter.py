...
class BiQuadFilter:
    def __init__(self, filter_type, **filter_params):
        ...
        if filter_type == "band_pass":
            omega = 2.0 * np.pi * filter_params['cutoff_frequency'] / filter_params['sampling_frequency']
            alpha = np.sin(omega) * np.sinh(np.log(2.0) / 2.0 * filter_params['cutoff_bandwidth'] * omega / np.sin(omega))
            self.a0 = 1.0 + alpha
            ...
            raise Exception("No filter named {}".format(filter_type))
    ...
    def apply_filter(self, current_input, key):
        # Initial process
        for stored_data in [self.last2_inputs, self.last1_inputs, self.last2_outputs, self.last1_outputs]:
...
class IMUFilterNode:
    def __init__(self):
        ...
        rospy.loginfo('a0: {}'.format(self.bqf.a0))
        rospy.loginfo('a1: {}'.format(self.bqf.a1))
        rospy.loginfo('a2: {}'.format(self.bqf.a2))
        rospy.loginfo('b0: {}'.format(self.bqf.b0))
        rospy.loginfo('b1: {}'.format(self.bqf.b1))
        rospy.loginfo('b2: {}'.format(self.bqf.b2))
    ...
    def imu_callback(self, msg):
        ...
        msg.linear_acceleration.x = self.bqf.apply_filter(msg.linear_acceleration.x, 'accel_x')
        msg.linear_acceleration.y = self.bqf.apply_filter(msg.linear_acceleration.y, 'accel_y')
        msg.linear_acceleration.z = self.bqf.apply_filter(msg.linear_acceleration.z, 'accel_z')
