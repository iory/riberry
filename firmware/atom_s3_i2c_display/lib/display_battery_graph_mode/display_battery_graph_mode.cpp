...
DisplayBatteryGraphMode::DisplayBatteryGraphMode()
  : Mode("DisplayBatteryGraphMode") {}
  ...
  while (true) {
    ...
    // Display information
      ...
      if (lcd.color_str.isEmpty()) {
        ...
        if (numParts > 0) {
          updateGraph(percentages, numParts-3, new_charge_status, charge_current, duration, lcd);
        }
...
void DisplayBatteryGraphMode::updateGraph(float* buffer, int buffer_length,
  ...
  for (int i = 0; i < buffer_length; i++) {
    ...
    for (int j = 0; j <= scaled_percentage; j++) {
      ...
      lcd.fillRect((y_label_w+y_line_w)+i*(buffer_w+gap), lcd.height()-(x_label_h+2)-j-1,
                                   buffer_w, 1, color);
    }
  }
  // x label text
...
void DisplayBatteryGraphMode::drawBatteryIcon(int x, int y, int width, int height, int batteryLevel, PrimitiveLCD &lcd) {
  int borderWidth = 2;
  ...
  lcd.fillRect(x + borderWidth, y + borderWidth,
